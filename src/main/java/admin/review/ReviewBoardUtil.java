...
public class ReviewBoardUtil {

  private static ReviewBoardUtil bu;

  private ReviewBoardUtil() {

  }

  public static ReviewBoardUtil getInstance() {

    if (bu == null) {
      bu = new ReviewBoardUtil();
    }
  ...
  public String pageNation(String url, String param, int totalPage, int currentPage) {
    ...
    while (movePage <= endPage) {
      if (movePage == currentPage) {// 현재 페이지에 대해서는 링크를 생성하지 않는다.
        pageNation.append("[<span style='font-size:20px'>").append(currentPage).append("</span>]");
      } else {
    ...
    if (totalPage > endPage) {
      movePage = endPage + 1;
      endMark = "[<a href='review_board_list.jsp?currentPage=" + movePage + "'> &gt;&gt; </a>]";
    }
